...
if ( !is_user_logged_in() && 'no' == get_option('woocommerce_enable_guest_checkout') ) { ?>
    <div class="login-reminder"><span><?php echo wp_kses_post( couponseek_get_field('login_to_buy_text', 'option') ? couponseek_get_field('login_to_buy_text', 'option') : __('Log in to Get Deal', 'couponseek')); ?></span></div>
<?php } elseif ( $product instanceof WC_Product_External && couponseek_get_field('discount_code') ) {
