...
// Remove Add To Cart if Guest checkout is disabled
remove_action( 'woocommerce_single_product_summary', 'woocommerce_template_single_add_to_cart', 30 );
add_action( 'woocommerce_single_product_summary', '_action_couponseek_single_add_to_cart', 30 );

if ( !function_exists('_action_couponseek_single_add_to_cart') ) {
	function _action_couponseek_single_add_to_cart( ) {
		global $product;

		if ( !is_user_logged_in() && 'no' == get_option('woocommerce_enable_guest_checkout') ) { ?>
			<div class="login-reminder"><span><?php echo wp_kses_post( couponseek_get_field('login_to_buy_text', 'option') ? couponseek_get_field('login_to_buy_text', 'option') : __('Log in to Get Deal', 'couponseek')); ?></span>
			</div>
		<?php } else {
	...
	function _filter_couponseek_wc_add_login_logout_register_menu( $nav, $args ) {
		...
		if ( couponseek_woocommerce() && !couponseek_get_field('hide_nav_menu_profile', 'option') ) {
			...
			<li class="<?php echo esc_attr($li_classes); ?>">
				...
				<?php else: ?>
					<a href="<?php echo esc_url(add_query_arg('redirect-to', get_the_permalink(), $url_login)); ?>" class="<?php echo esc_attr($a_classes); ?>">
						<div class="nav-icon">
							<svg viewBox="0 0 100 100" class="icon-svg">
								<use xlink:href="#icon-svg-user"></use>
							</svg>
