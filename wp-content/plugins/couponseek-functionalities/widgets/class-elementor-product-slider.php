...
class SSD_Elementor_Product_Slider_Widget extends \Elementor\Widget_Base {
	...
	public function get_script_depends() {
		return [ 'jquery-slick', 'couponseek_elementor-widgets-scripts' ];
	}
	...
	protected function render() {
		...
		$this->add_render_attribute( 'carousel', 'class', 'is-product-slider' );

		if ( 'none' !== $settings['navigation'] ) {
			if ( 'dots' !== $settings['navigation'] ) {
				$this->add_render_attribute( 'carousel', 'class', 'slick-arrows-outside' );
			}

			if ( 'arrows' !== $settings['navigation'] ) {
				$this->add_render_attribute( 'carousel', 'class', 'slick-dots-outside' );
			}
		}
	?>
	
		<div class="ProductSliderWrapper elementor-slick-slider" dir="<?php echo $settings['direction']; ?>">
			<?php 
				$products_ids = array();
			...
			<?php if ( count($products_ids) > 0 ) : ?>
				...
				<?php foreach ($products_ids as $product_id) : ?>
					<div class="product-slider-item pos-r">
						<?php 
						$product = wc_get_product($product_id);
						if ( !$product ) continue;
